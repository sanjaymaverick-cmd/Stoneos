...
import { shaClientOpId } from "../books/money";
    ...
    return sisters
      ...
      .map((s) => {
        const ar = invoices
          ...
          .reduce((sum, inv) => {
            const paid = inv.payments.reduce((sumPaid, row) => sumPaid + Number(row.amount), 0);
            const credited = inv.creditNotes.reduce((sumCn, row) => sumCn + Number(row.amount), 0);
  ...
  async payPayable(
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        const paidAt = parseFactoryDateInput(input.paidAt);
        let payment;
        try {
        ...
        const seller = await tx.factory.findUnique({ where: { id: payable.sellerFactoryId } });
        const sellerUser = {
          id: user.id,
          factoryId: payable.sellerFactoryId,
        } as AuthenticatedUser;
        await this.books.postPayment(tx, { ...user, ...sellerUser }, {
          paymentId: payment.id,
          invoiceId: invoice.id,
          customerName: seller?.name ?? "Sister plant",
          amount: input.amount,
          method: input.method,
          clientOpId: shaClientOpId([payable.sellerFactoryId, "if-receipt", input.clientOpId]),
          paidAt,
        });
        await this.books.postSisterSettlement(tx, {
          buyerFactoryId: user.factoryId,
          actorId: user.id,
          amount: input.amount,
          method: input.method,
          clientOpId: shaClientOpId([user.factoryId, "if-pay", input.clientOpId]),
          partyName: seller?.name ?? "Sister plant",
          paidAt,
        });
        await tx.auditEvent.create({
          data: {
