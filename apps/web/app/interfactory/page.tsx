...
export default function InterfactoryPage() {
  ...
  return (
    <AppShell>
      <h1>Sister plants</h1>
      <p>Link the other yards on this server. Sales to a sister customer post AP on their books. Settlement pays both AR and AP in one locked write. Stock stays on the selling plant (recovery is sale-time only).</p>
      ...
      <div className="card">
        ...
        {sisters.length === 0 ? <EmptyState>No other factories on this server yet. Create them above (up to the four plants).</EmptyState> : (
          <form onSubmit={link}>
            <label>Sister
      ...
      <div className="card">
        ...
        {payables.length === 0 ? <EmptyState>No interfactory bills yet. When a sister invoices this plant, the bill lands here.</EmptyState> : payables.map((p) => {
