...
**Status 2026-09-06 (code on `main`):** 1 volume + dump script landed (second-machine drill still open). 2 TZ + Z tests landed. 3 reversal endpoint + inventory UI landed (receipt / reservation / delivery only; PACKING skipped — `pack()` does not mutate stock). 4 deny-by-default + CSV lock + slab factory checks landed. 5 pay `FOR UPDATE` + stable per-form `clientOpId` landed; DB trigger also rejects overpay. 6 test globs + guard unit in CI landed; live HTTP CEO 403 is in `security-check.mjs` (run against smoke).

Books follow-ups on this pass: opening SoD uses line enterers; grinding/resin completion is not sellable; invoice numbers are factory+IST FY sequences; DPR good-slab totals come from slab rows; opening approve is batched with a 120s timeout; invoiced returns issue `CN-YYYY-NNNNN` credit notes; outbox queues on fetch failure and replays the stored `clientOpId`; ADR 0005 records application-WHERE isolation (not dual RLS); `baseVersion` mismatch is HTTP 409.
