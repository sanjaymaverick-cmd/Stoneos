...
**Status 2026-09-06 (code on `main`, HTTP on smoke):** 1 volume + dump script landed; second-*disk* rehearsal 5s to `E:/stoneos-backups` (`var/backup-rehearse.json`); second-*machine* drill still open (`scripts/restore-second-machine.sh` added 2026-09-08; must run on another PC). 2 TZ + Z tests landed. 3 reversal endpoint + inventory UI landed (receipt / reservation / delivery only; PACKING skipped — `pack()` does not mutate stock); HTTP: reverse requires reason 400, goods-receipt reverse voids + same `clientOpId` is a no-op. 4 deny-by-default + CSV lock + slab factory checks landed. 5 pay `FOR UPDATE` + stable per-form `clientOpId` + DB trigger; HTTP double-tap one payment row, overpay 400. 6 test globs + guard unit in CI landed; live HTTP CEO operator 403 / owner 200 / auditor 200 (`var/week-one-smoke.json`).

Books follow-ups on this pass (integration-tested): opening SoD uses line enterers; grinding/resin completion is not sellable; invoice numbers are factory+IST FY sequences (`INV-2026-00001` on smoke); DPR good-slab totals come from slab rows; opening approve is batched with a 120s timeout (150 lines); invoiced returns issue `CN-YYYY-NNNNN` credit notes; outbox queues on fetch failure and replays the stored `clientOpId`; ADR 0005 records application-WHERE isolation (not dual RLS); `baseVersion` mismatch is HTTP 409.
