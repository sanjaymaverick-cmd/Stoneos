...
- Isolation stays application `WHERE` (ADR 0005). Books copilot proposes drafts only (ADR 0015). Sister yards are ordinary customers/suppliers (ADR 0013 superseded). Cloud apply is workflow_dispatch only (ADR 0016). Live khata PDF parser is in-repo; paste the plant PDFs on smoke with `scripts/khata-cutover-smoke.sh` (do not log the owner password). Physical second-PC JSON and GSTN production secrets remain human-held.
